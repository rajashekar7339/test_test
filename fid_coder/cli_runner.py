...
from fid_coder import __version__, callbacks, plugins
...
def _render_turn_exception(exc: Exception) -> None:
    ...
    provider rate limit) are environment hiccups, not Fid Coder bugs. They get
    ...
    persisted to ``~/.fid_coder/logs/errors.log`` so SRE / support can still
    ...
    from fid_coder.messaging.queue_console import get_queue_console
...
async def main():
    """Main async entry point for Fid Coder CLI."""
    parser = argparse.ArgumentParser(description="Fid Coder - A code generation agent")
    parser.add_argument(
        "--version",
        "-v",
        action="version",
    ...
    parser.add_argument(
        ...
        "-a",
        type=str,
        help="Specify which agent to use (e.g., --agent fid-coder)",
    )
    parser.add_argument(
        "--model",
        "-m",
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="PATH",
        help="Resume a saved session from a .pkl file (e.g. ~/.fid_coder/contexts/foo.pkl)",
    )
    parser.add_argument(
        "--quick-resume",
        "-qr",
    ...
    initialize_command_history_file()
    from fid_coder.messaging import emit_error, emit_system_message

    # Show the awesome Fid Coder logo when entering interactive mode
    # This happens when: no -p flag (prompt-only mode) is used
    # The logo should appear for both `fid-coder` and `fid-coder -i`
    if not args.prompt:
        try:
            import pyfiglet
    ...
    # Load API keys from fid.cfg into environment variables
    ...
    if args.model:
        from fid_coder.config import _validate_model_exists
    ...
    # One-shot sweep of legacy ~/.fid_coder/contexts/ into the unified
    ...
    if args.resume:
        ...
        from fid_coder.config import AUTOSAVE_DIR, pin_current_session_name
    ...
    try:
        ...
        try:
            from fid_coder.messaging.run_ui import stop_persistent_ui
...
def _use_persistent_prompt() -> bool:
    ...
    False (→ classic prompt_toolkit path) when:
      * rollback flag: env FID_CODER_CLASSIC_PROMPT=1 or config
        ``classic_prompt`` truthy — protects the eyeball-testing period;
      * FID_CODER_NO_TUI=1 (tests / pexpect harnesses);
    ...
    if os.environ.get("FID_CODER_CLASSIC_PROMPT", "").strip().lower() in truthy:
        return False
    if os.environ.get("FID_CODER_NO_TUI", "").strip() == "1":
        return False
    try:
...
def _persistent_prompt_parts() -> tuple:
    ...
    try:
        ...
        from fid_coder.messaging.prompt_prefix_style import (
...
def _prompt_echo_text(task: str):
    ...
    from fid_coder.callbacks import on_prompt_text_color
...
def _interactive_sigint_guard(_sig, _frame):
    ...
    Ctrl+C in Fid Coder is a *cancel* gesture, never a *quit* gesture
    ...
    try:
        from fid_coder.terminal_utils import ensure_ctrl_c_disabled
    ...
    try:
        from fid_coder.messaging.run_ui import (
...
async def interactive_mode(message_renderer, initial_command: str = None) -> None:
    ...
    if initial_command:
        from fid_coder.agents import get_current_agent
        from fid_coder.messaging import emit_info, emit_success, emit_system_message
        ...
        try:
            ...
            if response is not None:
                ...
                from fid_coder.messaging import get_message_bus
                from fid_coder.messaging.messages import AgentResponseMessage
    ...
    if _use_persistent_prompt():
        try:
            from fid_coder.messaging.run_ui import start_persistent_ui
    ...
    while True:
        ...
        try:
            ...
            reset_windows_terminal_full()
            from fid_coder.callbacks import on_interactive_turn_cancel
            from fid_coder.messaging import emit_warning
        ...
        if task.strip():
            ...
            from fid_coder.config import auto_save_session_if_enabled
            ...
            from fid_coder.messaging import emit_system_message
...
async def run_prompt_with_attachments(
    ...
    *,
    ...
    from fid_coder.messaging import emit_system_message, emit_warning
    ...
    from fid_coder.agents.event_stream_handler import set_streaming_console
    ...
    if use_run_ui:
        ...
        from fid_coder.messaging.run_ui import run_ui
