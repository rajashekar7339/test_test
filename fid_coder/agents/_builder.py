...
entry point. Everything else in here (fid rules loading, MCP server loading,
...
hook; see :func:`fid_coder.callbacks.on_wrap_pydantic_agent`.
"""
...
def load_fid_rules() -> Optional[str]:
    ...
    **Search order for project rules:**
    ...
    # Priority 1: Check .fid_coder/ directory (preferred location)
    fid_coder_dir = Path(_FID_CODER_DIR)
    if fid_coder_dir.is_dir():
        for name in _AGENT_RULE_FILES:
            candidate = fid_coder_dir / name
            if candidate.exists():
...
def load_mcp_servers(
    ...
    from fid_coder.tools import tools_disabled
...
def _iter_autostart_targets(manager: Any, agent_name: str):
    ...
    """
    try:
        from fid_coder.mcp_.agent_bindings import get_bound_servers
...
def _assemble_instructions(agent: Any, resolved_model_name: str) -> str:
    """Compose full system prompt + fid rules + extended-thinking note."""
...
def build_pydantic_agent(
    ...
    - ``agent._fid_rules = None`` (invalidates any cached rules)
