...
def _handle_copilot_login(command: str) -> None:
    ...
    # Use plain input() (via safe_input) — prompt_toolkit.prompt() calls
    # asyncio.run() and blows up inside the interactive event loop.
    if not host:
        emit_info(
            ...
            "   Tip: /copilot-login github.com skips this prompt."
        )
        try:
            from fid_coder.command_line.utils import safe_input

            raw = safe_input("GitHub host [github.com]: ").strip()
