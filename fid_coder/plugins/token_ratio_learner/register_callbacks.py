...
def _patched_estimate_tokens(text: str) -> int:
    ...
    """
    from fid_coder.plugins.token_ratio_learner.ratios import count_tokens
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if input_tokens > 0 and input_char_count > 0:
            from fid_coder.plugins.token_ratio_learner.ratios import (
                _record_token_ratio,
            )
...
def _on_startup() -> None:
    ...
    from fid_coder.agents import _history
    ...
    # NOTE: ``fid_coder.agents.subagent_stream_handler`` re-exports the
    # *function* ``subagent_stream_handler``, not the module.  We must
    # reach the actual module via ``importlib``.
    import importlib

    ssh_mod = importlib.import_module("fid_coder.agents.subagent_stream_handler")
