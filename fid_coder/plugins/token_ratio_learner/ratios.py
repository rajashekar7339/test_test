...
Storage lives at ``~/.fid_coder/token_ratios.json``, overridable via
the env var ``FID_CODER_TOKEN_RATIOS_PATH``.
"""
...
_TOKEN_RATIOS_PATH: Path = Path(
    os.path.expanduser(
        os.environ.get(
            "FID_CODER_TOKEN_RATIOS_PATH",
            str(Path.home() / ".fid_coder" / "token_ratios.json"),
        )
    )
)
