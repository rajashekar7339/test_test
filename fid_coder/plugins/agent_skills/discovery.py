...
from fid_coder.callbacks import get_callbacks, on_register_skills
from fid_coder.config import CACHE_DIR
from fid_coder.plugins.agent_skills.config import get_skill_directories
...
def get_default_skill_directories() -> List[Path]:
    ...
    Returns:
        - ~/.fid_coder/skills (user skills)
        - ./.fid_coder/skills (project config skills)
        - ./skills (project skills)
    """
    return [
        Path.home() / ".fid_coder" / "skills",
        Path.cwd() / ".fid_coder" / "skills",
        Path.cwd() / "skills",
    ]
