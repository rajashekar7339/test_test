...
from fid_coder.plugins.agent_skills.discovery import refresh_skill_cache
from fid_coder.plugins.agent_skills.installer import InstallResult

logger = logging.getLogger(__name__)

_DEFAULT_SKILLS_DIR = Path.home() / ".fid_coder" / "skills"
...
def _download_to_file(url: str, dest: Path) -> bool:
    ...
    headers = {
        ...
        "User-Agent": "fid-coder/skill-downloader",
    }
...
def download_and_install_skill(
    ...
    Args:
        ...
        target_dir: Base skills directory. Defaults to ~/.fid_coder/skills.
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="fid_coder_skill_") as tmp:
            tmp_dir = Path(tmp)
