...
description: How Fid Coder itself is built — its internal architecture, structure, codebase layout, and source modules. Explains agents, tools, the plugin/callback hook system, models, MCP, sessions and history/context windows, skills, slash commands, config, messaging/UI, system-prompt assembly. Activate for ANY question about how Fid Coder works internally, why it behaves a certain way, where something lives in the code, how a feature is implemented, or how to navigate, debug, or extend the codebase (add a tool, agent, plugin, command, skill, model, or MCP server).
version: "1.1"
author: fid-coder
tags:
  - fid-coder
...
# Fid Coder — Architecture & Internals

This skill is a **self-awareness document**. When activated it teaches the
LLM how Fid Coder is structured under the hood so it can navigate the
codebase, debug issues, and extend the product without guessing.

> **Philosophy:** Fid Coder is plugin-first. Nearly all new functionality
> should be a plugin under `fid_coder/plugins/` that hooks into core via
> `fid_coder/callbacks.py`. Don't edit `fid_coder/command_line/` or core
...
| Member | Purpose |
|--------|---------|
| `name` (abstract property) | Stable machine identifier, e.g. `"fid-coder"` |
...
**JSON agents** — `JSONAgent` loads from `~/.fid_coder/agents/*.json`
(user) and `<CWD>/.fid_coder/agents/*.json` (project). Project overrides
...
|------|----------|------------|
| **Builtin** | `fid_coder/plugins/<name>/register_callbacks.py` | 1st |
| **User** | `~/.fid_coder/plugins/<name>/register_callbacks.py` | 2nd |
| **Project** | `<CWD>/.fid_coder/plugins/<name>/register_callbacks.py` | 3rd (highest precedence) |
...
**Most important hooks for extending Fid Coder:**
...
2. **`~/.fid_coder/extra_models.json`** — user-defined custom models
...
Fid Coder manages external MCP (Model Context Protocol) servers that
...
```
4. AGENTS.md fid_rules  (global ~/.fid_coder/AGENTS.md, then project
   <CWD>/.fid_coder/AGENTS.md, then ./AGENTS.md fallback — concatenated,
   not first-wins)
...
All settings live in `~/.fid_coder/fid.cfg` (INI format) managed via:
- `get_value(key)` / `set_value(key, value)`
- `/set <key> <value>` slash command

Key directories:
- **Config root**: `~/.fid_coder/`
- **State/sessions**: `~/.fid_coder/state/` (or platform cache dir)
- **Cache**: `~/.fid_coder/cache/`
- **Agents**: `~/.fid_coder/agents/`
- **Skills**: `~/.fid_coder/skills/`
- **Plugins**: `~/.fid_coder/plugins/`
- **Extra models**: `~/.fid_coder/extra_models.json`

AGENTS.md rules files are loaded from `~/.fid_coder/AGENTS.md` (global),
`<CWD>/.fid_coder/AGENTS.md` (project), and `./AGENTS.md` (fallback).
...
```python
from fid_coder.messaging import emit_info, emit_success, emit_warning, emit_error
...
- **Extend Fid Coder** with custom tools, agents, skills, or commands
