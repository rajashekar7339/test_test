...
*new line* below the fid/agent/model/cwd chrome, so long working-directory
...
* ``custom_command`` / ``custom_command_help`` — exposes ``/prompt_newline``
  for runtime on/off, persisted via ``fid.cfg``.
...
from fid_coder.plugins.prompt_newline.config import is_enabled, set_enabled
