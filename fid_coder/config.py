...
from fid_coder.session_storage import save_session
...
def _get_xdg_dir(env_var: str, fallback: str) -> str:
    """
    Get directory for fid_coder files, defaulting to ~/.fid_coder.

    XDG paths are only used when the corresponding environment variable
    is explicitly set by the user. Otherwise, we use the legacy ~/.fid_coder
    ...
    Returns:
        Path to the directory for fid_coder files
    """
    # Use XDG directory ONLY if environment variable is explicitly set
    xdg_base = os.getenv(env_var)
    if xdg_base:
        return os.path.join(xdg_base, "fid_coder")

    # Default to legacy ~/.fid_coder for all file types
    return os.path.join(os.path.expanduser("~"), ".fid_coder")
...
CONFIG_FILE = os.path.join(CONFIG_DIR, "fid.cfg")
...
# Copilot models registered after /copilot-login (XDG_DATA_HOME)
...
def get_mcp_unbound_warning_silenced() -> bool:
    ...
    When True, ``fid_coder.mcp_.manager._warn_unbound_servers`` skips emitting
...
def get_retry_main_strategy() -> str:
    ...
    Delegates to :func:`fid_coder.agents.retry_profiles.resolve` so the value
...
DEFAULT_SECTION = "fid"
REQUIRED_KEYS = ["fid_name", "owner_name"]
...
def ensure_config_exists():
    """
    Ensure that XDG directories and fid.cfg exist, prompting if needed.
    ...
    if missing:
        ...
        sys.stdout.write("🐾 Let's get your Fid ready!\n")
        sys.stdout.flush()
        for key in missing:
            if key == "fid_name":
                val = input("What should we name the fid? ").strip()
            elif key == "owner_name":
                val = input("What's your name (so Fid Coder knows its owner)? ").strip()
            else:
...
def get_fid_name():
    return get_value("fid_name") or "Fid"
...
def get_config_keys():
    """
    Returns the list of all config keys currently in fid.cfg,
...
def load_mcp_server_configs():
    ...
    1. **User-level** \u2014 ``$XDG_CONFIG_HOME/fid_coder/mcp_servers.json``
       (global, always trusted).
    2. **Project-level** \u2014 ``<CWD>/.fid_coder/mcp_servers.json``, but ONLY
       ...
       see :mod:`fid_coder.mcp_.project_config`.
    ...
    try:
        from fid_coder.mcp_.project_config import load_project_mcp_server_configs
...
def model_supports_setting(model_name: str, setting: str) -> bool:
    ...
    if setting in ("thinking_type", "clear_thinking"):
        from fid_coder.model_utils import supports_glm_thinking

        if supports_glm_thinking(model_name):
            return True
    if setting == "glm_reasoning_effort":
        from fid_coder.model_utils import supports_glm_reasoning_effort
    ...
    try:
        ...
        if supported_settings is None:
            ...
            if model_name.startswith("claude-") or model_name.startswith("anthropic-"):
                ...
                from fid_coder.model_utils import supports_adaptive_thinking
...
def _warn_no_model_available() -> None:
    ...
    try:
        ...
        emit_warning(
            "\u26a0\ufe0f  No model is configured! Fid Coder can't talk to an LLM "
            "until you sign in with GitHub Copilot.\n"
            "   \u2022 Run /copilot-login to authenticate and register models."
        )
    except Exception:
...
def get_global_model_name():
    """Return the model name for Fid Coder to use, or ``None`` if unset.
    ...
    2. Otherwise, look at ``model`` in *fid.cfg*
...
def get_fid_token():
    """Returns the fid_token from config, or None if not set."""
    return get_value("fid_token")


def set_fid_token(token: str):
    """Sets the fid_token in the persistent config file."""
    set_config_value("fid_token", token)
...
def get_user_agents_directory() -> str:
    ...
    Returns:
        Path to the user's Fid Coder agents directory.
    """
...
def get_project_agents_directory() -> Optional[str]:
    ...
    Looks for a .fid_coder/agents/ directory in the current working directory.
    ...
    """
    project_agents_dir = os.path.join(os.getcwd(), ".fid_coder", "agents")
    ...
    if not command_history_exists:
        try:
            ...
            old_history_file = os.path.join(
                os.path.expanduser("~"), ".fid_coder_history.txt"
            )
...
def get_safety_permission_level():
    """
    Checks fid.cfg for 'safety_permission_level' (case-insensitive in value only).
...
def get_mcp_disabled():
    """
    Checks fid.cfg for 'disable_mcp' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
    When enabled, Fid Coder will skip loading MCP servers entirely.
    """
    true_vals = {"1", "true", "yes", "on"}
...
def get_grep_output_verbose():
    """
    Checks fid.cfg for 'grep_output_verbose' (case-insensitive in value only).
...
def get_disable_dangerous_command_guard() -> bool:
    """
    Checks fid.cfg for 'disable_dangerous_command_guard' (case-insensitive in value only).
...
def get_auto_save_session() -> bool:
    """
    Checks fid.cfg for 'auto_save_session' (case-insensitive in value only).
...
DEFAULT_BANNER_COLORS = {
    "thinking": "slate_blue3",  # Indigo - contemplation
    "agent_response": "orchid",  # Orchid - main AI output
    "shell_command": "khaki3",  # Amber - system commands
    "read_file": "cadet_blue",  # Cadet - reading files
    "edit_file": "light_goldenrod3",  # Gold - modifications (legacy)
    "create_file": "light_goldenrod3",  # Gold - file creation
    "replace_in_file": "light_goldenrod3",  # Gold - file modifications
    "delete_snippet": "light_goldenrod3",  # Gold - snippet removal
    "grep": "grey42",  # Silver - search results
    "directory_listing": "sky_blue3",  # Sky - navigation
    "agent_reasoning": "medium_orchid",  # Orchid - deep thought
    "invoke_agent": "hot_pink3",  # Rose - agent invocation
    "subagent_response": "spring_green3",  # Jade - sub-agent success
    "list_agents": "light_slate_grey",  # Slate - neutral listing
    "universal_constructor": "turquoise4",  # Turquoise - constructing tools
    # Browser/Terminal tools - same color as edit_file (gold)
    "terminal_tool": "light_goldenrod3",  # Gold - browser terminal operations
    # MCP tools - distinct from builtin tools
    "mcp_tool_call": "turquoise4",  # Turquoise - external MCP tool calls
    # User-initiated shell pass-through (! prefix) - distinct from agent's shell_command
    "shell_passthrough": "pale_green3",  # Green - user's own shell commands
    # LLM Judge - goal-mode verdict (distinct from agent reasoning)
    "llm_judge": "orange3",  # Amber - judicial authority / gavel
}
...
def pin_current_session_name(name: str) -> str:
    ...
    """
    from fid_coder.session_lifecycle import is_valid_session_name
...
def auto_save_session_if_enabled() -> bool:
    ...
    try:
        ...
        stats_suffix = ""
        try:
            from fid_coder.agents.run_stats import AgentRunStats
        ...
        from fid_coder.session_lifecycle import fire_post_autosave_callback
...
def record_terminal_session(session_name: str, *, overwrite: bool = True) -> None:
    ...
    reassignment while Fid Coder is running is rare, but possible after a
...
def observe_quick_resume_path(target_path: str, *, path_kind: str = "auto") -> bool:
    ...
    even when Fid Coder was launched from the parent directory.
    """
...
def get_suppress_thinking_messages() -> bool:
    """
    Checks fid.cfg for 'suppress_thinking_messages' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
...
def get_smooth_thinking_stream() -> bool:
    """
    Checks fid.cfg for 'smooth_thinking_stream' (case-insensitive in value only).
    Defaults to True if not set.
...
def get_smooth_response_stream() -> bool:
    """
    Checks fid.cfg for 'smooth_response_stream' (case-insensitive in value only).
    Defaults to True if not set.
    Allowed values for OFF: 0, '0', 'false', 'no', 'off' (all case-insensitive).
...
def get_suppress_informational_messages() -> bool:
    """
    Checks fid.cfg for 'suppress_informational_messages' (case-insensitive in value only).
    Defaults to False if not set.
    Allowed values for ON: 1, '1', 'true', 'yes', 'on' (all case-insensitive for value).
...
def get_output_level() -> str:
    ...
    (current behaviour).  The value is read from ``fid.cfg`` with the
    key ``output_level``.
...
def load_api_keys_to_environment():
    """Load all API keys from .env and fid.cfg into environment variables.

    Priority order:
    1. .env file (highest priority) - if present in current directory
    2. fid.cfg - fallback if not in .env
    ...
    # providers). Without this, such keys saved in fid.cfg never hydrate into
    # os.environ at startup. Best-effort: never let discovery break startup.
    try:
        from fid_coder.provider_credentials import all_required_env_vars
    ...
    # Step 2: Load from fid.cfg, but only if not already set
    # This ensures .env has priority over fid.cfg
...
def get_default_agent() -> str:
    """
    Get the default agent name from fid.cfg.

    Returns:
        str: The default agent name, or "fid-coder" if not set.
    """
    return get_value("default_agent") or "fid-coder"


def set_default_agent(agent_name: str) -> None:
    """
    Set the default agent name in fid.cfg.
