"""Command handlers for Fid Coder - CORE commands.
...
from fid_coder.command_line.utils import make_directory_table
from fid_coder.config import finalize_autosave_session
from fid_coder.messaging import emit_error, emit_info
from fid_coder.tools.tools_content import tools_content
...
def handle_tutorial_command(command: str) -> bool:
    ...
    if result == "copilot":
        emit_info("🔐 Starting GitHub Copilot login...")
...
def handle_mcp_command(command: str) -> bool:
    ...
    from fid_coder.command_line.mcp import MCPCommandHandler
...
def handle_undo_command(command: str) -> bool:
    ...
    from fid_coder.messaging import emit_info, emit_error
