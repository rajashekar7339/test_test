...
class SetCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        from fid_coder.command_line.config_apply import MODEL_SETTINGS_ONLY_KEYS

        config_keys = sorted(get_config_keys())

        for key in config_keys:
            if key in {"model", "fid_token"} | MODEL_SETTINGS_ONLY_KEYS:
                continue  # exclude 'model' and 'fid_token' from regular /set completions
...
class SlashCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        try:
            from fid_coder import callbacks, plugins
...
def get_prompt_with_active_model(base: str = "\u276f "):
    ...
    agent_display = current_agent.display_name if current_agent else "fid-coder"
    ...
    return FormattedText(
        [
            ("class:fid class:tui.header", f"{fid}"),
            ("", " "),
            (
