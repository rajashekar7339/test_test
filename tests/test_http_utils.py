...
from fid_coder.http_utils import ProxyConfig
...
class TestResolveProxyConfig:
    ...
    @patch("fid_coder.http_utils.get_cert_bundle_path")
    @patch("fid_coder.http_utils.get_http2")
    ...
    def test_resolve_disable_retry_transport(
        ...
        with patch.dict(os.environ, {"FID_CODER_DISABLE_RETRY_TRANSPORT": "1"}):
            config = _resolve_proxy_config()
    ...
    def test_resolve_disable_retry_transport_case_insensitive(
        ...
        for value in ["1", "true", "yes", "True", "YES"]:
            with patch.dict(os.environ, {"FID_CODER_DISABLE_RETRY_TRANSPORT": value}):
                config = _resolve_proxy_config()
                assert config.disable_retry is True
...
class TestCreateAsyncClient:
    ...
    @patch("fid_coder.http_utils._resolve_proxy_config")
    def test_create_async_client_basic(
        self,
        mock_resolve_proxy,
    ):
        """Test basic async client creation."""
        from fid_coder.http_utils import create_async_client
