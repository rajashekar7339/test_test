...
from fid_coder.plugins.wiggum import judge_config, register_callbacks
from fid_coder.plugins.wiggum.judge import GoalJudgement
from fid_coder.plugins.wiggum.judge_config import JudgeConfig
...
def _fake_agent(name: str = "fid-coder", history: list | None = None):
    agent = MagicMock()
    agent.name = name
...
async def test_judge_exception_becomes_abstaining_verdict(isolated_judges):
    ...
    with (
        patch.object(register_callbacks, "judge_goal", new=fake_judge_goal),
        patch.object(register_callbacks, "_display_llm_judge"),
        patch("fid_coder.error_logging.log_error"),
    ):
        all_complete, notes, verdicts = await register_callbacks._run_goal_judges(
            agent=_fake_agent(),
            goal="g",
...
async def test_turn_end_feeds_remediation_notes_to_next_iteration(isolated_judges):
    ...
    from fid_coder.plugins.wiggum import state
...
async def test_judge_runs_inside_subagent_context(isolated_judges):
    ...
    """
    from fid_coder.plugins.wiggum import judge as judge_module
