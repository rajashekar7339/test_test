...
from fid_coder.plugins.fork import register_callbacks as rc

_AGENTS = {"fid-coder": "the default pup", "qa-kitten": "meow"}
...
def fake_agent_manager():
    with (
        patch(
            "fid_coder.agents.agent_manager.get_available_agents",
...
def test_started_message_is_rich_and_theme_aware():
    from fid_coder.messaging.rich_renderer import DEFAULT_STYLES
...
def test_response_message_identifies_fork_and_renders_markdown():
    ...
    with patch("fid_coder.config.get_banner_color", return_value="dark_orange3"):
...
async def test_fork_success_emits_response_and_completion_banner():
    ...
    with (
        patch(
            "fid_coder.tools.subagent_invocation._invoke_agent_impl",
            new=_fake_impl(),
        ),
...
async def test_fork_publishes_completion_lifecycle_event():
    ...
    with (
        ...
        patch("fid_coder.callbacks.on_post_tool_call", new=capture),
...
async def test_fork_defaults_to_current_agent():
    ...
    record = next(iter(rc._forks.values()))
    assert record.agent_name == "fid-coder"
...
async def test_fork_reports_crash_as_failure():
    ...
    errors = []
    with (
        patch("fid_coder.tools.subagent_invocation._invoke_agent_impl", new=boom),
        patch.object(rc, "_emit_info"),
        patch.object(rc, "_emit_error", errors.append),
    ):
...
async def test_fork_cancel_running_fork():
    ...
    warnings = []
    with (
        patch("fid_coder.tools.subagent_invocation._invoke_agent_impl", new=slow),
        patch.object(rc, "_emit_info"),
