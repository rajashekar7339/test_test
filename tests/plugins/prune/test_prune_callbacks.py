"""Tests for ``fid_coder.plugins.prune.register_callbacks``.
...
class TestPerformPrune:
    def test_empty_history_emits_warning(self):
        ...
        with (
            ...
            patch(
                "fid_coder.plugins.prune.register_callbacks.emit_warning"
            ) as mock_warn,
        ):
    ...
    def test_nothing_selected_emits_info(self):
        ...
        with (
            ...
            patch("fid_coder.plugins.prune.register_callbacks.emit_info") as mock_info,
        ):
    ...
    def test_pure_user_first_message_is_prunable(self):
        ...
        with (
            ...
            patch("fid_coder.plugins.prune.register_callbacks.emit_success"),
            patch("fid_coder.plugins.prune.register_callbacks.emit_info"),
        ):
            _plugin_module()._perform_prune({0})
    ...
    def test_set_history_failure_emits_error(self):
        ...
        with (
            ...
            patch(
                "fid_coder.plugins.prune.register_callbacks.emit_error"
            ) as mock_error,
        ):
            _plugin_module()._perform_prune({1})
...
class TestHandlePruneCommand:
    ...
    def test_only_system_entries_bails_out(self):
        ...
        agent.get_full_system_prompt.return_value = "you are a fid"
        with (
            patch.dict(
                sys.modules,
                {"fid_coder.agents.agent_manager": _agent_manager_module(agent)},
    ...
    def test_get_agent_failure_emits_error(self):
        ...
        with (
            patch.dict(sys.modules, {"fid_coder.agents.agent_manager": bad_manager}),
            patch(
                "fid_coder.plugins.prune.register_callbacks.emit_error"
            ) as mock_error,
        ):
