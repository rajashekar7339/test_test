...
def _plugin_module():
    return importlib.import_module("fid_coder.plugins.prune.register_callbacks")
...
def _system_msg() -> ModelRequest:
    return ModelRequest(parts=[SystemPromptPart(content="you are a fid")])


def _system_plus_user_msg(
    system: str = "you are a fid", user: str = "first turn"
) -> ModelRequest:
...
def _menu_with_history():
    ...
    """
    from fid_coder.plugins.prune.prune_menu import PruneMenu
    from fid_coder.plugins.prune.prune_model import build_message_entries
