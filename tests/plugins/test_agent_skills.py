...
from fid_coder.callbacks import clear_callbacks, register_callback
from fid_coder.plugins.agent_skills.config import (
    add_skill_directory,
    get_disabled_skills,
...
from fid_coder.plugins.agent_skills.discovery import (
    SkillInfo,
    discover_skills,
...
from fid_coder.plugins.agent_skills.metadata import (
    SkillMetadata,
    get_skill_resources,
...
from fid_coder.plugins.agent_skills.prompt_builder import (
...
class TestSkillDiscovery:
    ...
    def test_get_default_skill_directories(self):
        ...
        assert directories[0] == Path.home() / ".fid_coder" / "skills"
        assert directories[1] == Path.cwd() / ".fid_coder" / "skills"
    ...
    def test_discover_skills_deduplicates_across_directories(self, tmp_path, caplog):
        ...
        ~/.fid_coder/skills/foo and ~/.claude/skills/foo), only the first one
    ...
    def test_refresh_skill_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "fid_coder.plugins.agent_skills.discovery.discover_skills", mock_discover
        )
...
class TestSkillsConfig:
    ...
    def test_get_skill_directories_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "fid_coder.plugins.agent_skills.config.get_value", mock_get_value
        )

        directories = get_skill_directories()
        assert len(directories) == 3
        # The tilde will be expanded to the actual home directory
        assert ".fid_coder/skills" in directories[0]
        assert ".fid_coder/skills" in directories[1]
    ...
    def test_add_skill_directory_new(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "fid_coder.plugins.agent_skills.config.set_value", mock_set_value
        )
