...
def _plugin_module():
    return importlib.import_module("fid_coder.plugins.wiggum.register_callbacks")
...
def test_judges_command_invokes_menu():
    ...
    with (
        ...
        patch(
            "fid_coder.command_line.judges_menu.interactive_judges_menu"
        ) as mock_menu,
    ):
...
def test_prompt_extraction_is_command_word_agnostic():
    ...
    from fid_coder.plugins.wiggum.register_callbacks import _extract_prompt
...
def test_usage_hint_mentions_aliases():
    ...
    from fid_coder.plugins.wiggum.register_callbacks import handle_goal_command
    ...
    with (
        patch(
            "fid_coder.plugins.wiggum.register_callbacks.emit_warning",
            side_effect=capture,
        ),
        patch(
            "fid_coder.plugins.wiggum.register_callbacks.emit_info",
            side_effect=capture,
        ),
    ):
