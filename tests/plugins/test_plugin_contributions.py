...
from fid_coder.plugins.plugin_list import plugin_contributions as pc
...
def test_plugin_owner_of_module_core_does_not_match_plugin():
    ...
    assert (
        pc._plugin_owner_of_module("fid_coder.command_line.command_handler")
        == "fid_coder"
    )
...
def test_registry_commands_attributed_by_handler_module(monkeypatch):
    from fid_coder.command_line import command_registry
    ...
    infos = [
        ...
        _FakeCommandInfo(
            ...
            _handler_in("fid_coder.plugins.elsewhere.register_callbacks"),
        ),
        _FakeCommandInfo(
            "core_cmd",
            "A core command",
            _handler_in("fid_coder.command_line.command_handler"),
        ),
    ]
    ...
    monkeypatch.setattr(
        ...
        lambda: [
            _FakeCommandInfo(
                "decorated",
                "From the decorator",
                _handler_in("fid_coder.plugins.plugA.register_callbacks"),
            )
        ],
    )
    assert pc.get_commands("plugA") == [
...
def test_registry_tools_attributed_by_register_func_module(monkeypatch):
    import fid_coder.tools as tools_pkg

    fake_registry = {
        "plugA_tool": _handler_in("fid_coder.plugins.plugA.register_callbacks"),
        "core_tool": _handler_in("fid_coder.tools.file_operations"),
    }
    monkeypatch.setattr(tools_pkg, "TOOL_REGISTRY", fake_registry, raising=False)

    assert pc._registry_tools("plugA") == ["plugA_tool"]
    assert pc._registry_tools("fid_coder") == ["core_tool"]
...
def test_get_tools_merges_callback_and_registry(clean_callbacks, monkeypatch):
    ...
    monkeypatch.setattr(
        tools_pkg,
        "TOOL_REGISTRY",
        {"direct_tool": _handler_in("fid_coder.plugins.plugA.register_callbacks")},
        raising=False,
    )
