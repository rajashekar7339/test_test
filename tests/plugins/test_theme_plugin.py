...
from fid_coder.plugins.theme.themes import (
    CURATED_THEMES,
    DEFAULT,
    MENU,
    MENU_BY_INDEX,
...
from fid_coder.plugins.theme.bundled_palettes import (
    BUBBLEGUM_PINK,
    CATPPUCCIN_LATTE,
    CATPPUCCIN_MOCHA,
    DEEP_BLACK,
...
from fid_coder.plugins.theme.picker import (
    THEMES_PER_PAGE,
    _format_menu,
    _move_page,
    _page_for_index,
...
from fid_coder.plugins.theme.rich_themes import make_remap, _swap_color, _safe_parse
from fid_coder.plugins.theme.content_styles import (
    CONTENT_KEYS,
...
from fid_coder.plugins.theme.osc_palette import (
    _osc,
    BEL,
    ESC,
    get_saved_palette,
...
class TestThemeCatalog:
    ...
    def test_menu_by_index_maps_strings(self):
        ...
        assert MENU_BY_INDEX["6"] == "purple-fid"
    ...
    def test_aliases_resolve(self):
        ...
        assert MENU_BY_NAME["fid"] is CURATED_THEMES["purple-fid"]
        assert MENU_BY_NAME["purple"] is CURATED_THEMES["purple-fid"]
...
class TestBundledPalettes:
    ...
    def test_purple_fid_muted_text_has_accessible_contrast(self):
        """ANSI bright black is muted TUI text and must remain readable."""
        assert _contrast_ratio(PURPLE_FID["bg"], PURPLE_FID["ansi"][8]) >= 4.5
...
class TestOscPalette:
    ...
    def test_apply_palette_emits_sequences(self):
        ...
        with patch("fid_coder.plugins.theme.osc_palette._emit") as mock_emit:
    ...
    def test_get_saved_palette_returns_none_when_empty(self):
        with patch("fid_coder.plugins.theme.osc_palette.get_value", return_value=None):
    ...
    def test_get_saved_palette_returns_dict(self):
        ...
        data = {"bg": "#000", "fg": "#fff"}
        with patch(
            "fid_coder.plugins.theme.osc_palette.get_value",
...
class TestRegisterCallbacks:
    def test_prompt_text_uses_active_terminal_foreground(self):
        from fid_coder.plugins.theme.register_callbacks import _prompt_text_color

        with patch(
            "fid_coder.plugins.theme.register_callbacks._active_terminal_palette",
    ...
    def test_green_screen_highlighter_removes_monokai_white(self):
        ...
        from fid_coder.plugins.theme.register_callbacks import _termflow_highlighter
    ...
    def test_termflow_style_uses_active_terminal_palette(self):
        ...
        from fid_coder.plugins.theme.register_callbacks import _termflow_style

        default = RenderStyle.default()
        with (
            patch(
                "fid_coder.plugins.theme.register_callbacks.get_value",
                return_value="green-screen",
            ),
            patch("fid_coder.plugins.theme.register_callbacks.osc") as mock_osc,
        ):
    ...
    def test_first_run_applies_purple_fid(self):
        ...
        with (
            ...
            patch("fid_coder.plugins.theme.register_callbacks.apply") as mock_apply,
            patch("fid_coder.plugins.theme.register_callbacks.cs") as mock_cs,
            patch("fid_coder.plugins.theme.register_callbacks.rt") as mock_rt,
            patch("fid_coder.plugins.theme.register_callbacks.osc") as mock_osc,
            patch(
                "fid_coder.plugins.theme.register_callbacks.set_config_value"
            ) as mock_set,
        ):
    ...
    def test_custom_help_returns_theme_entry(self):
        from fid_coder.plugins.theme.register_callbacks import _custom_help

        entries = dict(_custom_help())
        assert "theme" in entries

    def test_handle_theme_ignores_other_commands(self):
        from fid_coder.plugins.theme.register_callbacks import _handle_theme
    ...
    def test_handle_theme_show(self):
        ...
        with patch("fid_coder.plugins.theme.register_callbacks.emit_info") as mock_info:
    ...
    def test_handle_theme_unknown_warns(self):
        ...
        with patch(
            "fid_coder.plugins.theme.register_callbacks.emit_warning"
        ) as mock_warn:
    ...
    def test_handle_theme_by_name_applies(self):
        ...
        with (
            patch("fid_coder.plugins.theme.register_callbacks.apply") as mock_apply,
            patch("fid_coder.plugins.theme.register_callbacks.cs"),
            patch("fid_coder.plugins.theme.register_callbacks.rt"),
            patch("fid_coder.plugins.theme.register_callbacks.osc"),
            patch("fid_coder.plugins.theme.register_callbacks.set_config_value"),
            patch("fid_coder.plugins.theme.register_callbacks.emit_info"),
        ):
    ...
    def test_handle_theme_interactive_cancel(self):
        ...
        with (
            patch(
                "fid_coder.plugins.theme.register_callbacks._run_interactive_picker",
                return_value=None,
            ),
            patch("fid_coder.plugins.theme.register_callbacks.emit_info") as mock_info,
        ):
