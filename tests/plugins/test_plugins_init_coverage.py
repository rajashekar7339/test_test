"""Comprehensive tests for fid_coder/plugins/__init__.py.

Tests cover plugin loading functions including:
- Built-in plugin loading with various edge cases
- User plugin loading from ~/.fid_coder/plugins/
...
class TestGetUserPluginsDir:
    ...
    def test_returns_user_plugins_path(self):
        ...
        assert result == Path.home() / ".fid_coder" / "plugins"
...
class TestEnsureUserPluginsDir:
    ...
    def test_creates_directory_if_not_exists(self, tmp_path):
        ...
        test_dir = tmp_path / ".fid_coder" / "plugins"
...
class TestLoadBuiltinPlugins:
    ...
    def test_loads_valid_plugin(self, tmp_path):
        ...
        with (
            patch("fid_coder.config.get_safety_permission_level", return_value="high"),
            patch("fid_coder.plugins.importlib.import_module") as mock_import,
        ):
            result = _load_builtin_plugins(tmp_path)
            assert "my_plugin" in result
            mock_import.assert_called_once_with(
                "fid_coder.plugins.my_plugin.register_callbacks"
            )
    ...
    def test_skips_shell_safety_when_safety_level_medium(self, tmp_path):
        ...
        with (
            patch(
                "fid_coder.config.get_safety_permission_level", return_value="medium"
            ),
            patch("fid_coder.plugins.importlib.import_module") as mock_import,
        ):
            result = _load_builtin_plugins(tmp_path)
    ...
    def test_loads_shell_safety_when_safety_level_low(self, tmp_path):
        ...
        with (
            patch("fid_coder.config.get_safety_permission_level", return_value="low"),
            patch("fid_coder.plugins.importlib.import_module") as mock_import,
        ):
            result = _load_builtin_plugins(tmp_path)
            assert "shell_safety" in result
            mock_import.assert_called_once_with(
                "fid_coder.plugins.shell_safety.register_callbacks"
            )
    ...
    def test_loads_shell_safety_when_safety_level_none(self, tmp_path):
        ...
        with (
            patch("fid_coder.config.get_safety_permission_level", return_value="none"),
    ...
    def test_handles_import_error(self, tmp_path, caplog):
        ...
        with (
            patch("fid_coder.config.get_safety_permission_level", return_value="high"),
            patch(
                "fid_coder.plugins.importlib.import_module",
...
class TestLoadPluginCallbacks:
    ...
    def test_calls_all_three_load_functions(self, tmp_path):
        ...
        with (
            patch(
                "fid_coder.plugins._load_builtin_plugins",
                return_value=["builtin_plugin"],
            ) as mock_builtin,
            patch(
                "fid_coder.plugins._load_user_plugins", return_value=["user_plugin"]
            ) as mock_user,
            patch(
            ...
            patch(
                "fid_coder.plugins._load_project_plugins",
    ...
    def test_skips_project_when_dir_missing(self):
        ...
        with (
            ...
            patch("fid_coder.plugins._load_user_plugins", return_value=[]),
            patch(
                "fid_coder.plugins.get_project_plugins_directory",
                return_value=None,
            ),
    ...
    def test_logs_loaded_plugins(self, caplog):
        ...
        with (
            ...
            patch("fid_coder.plugins._load_user_plugins", return_value=["test_user"]),
            patch(
                "fid_coder.plugins.get_project_plugins_directory",
                return_value=None,
            ),
