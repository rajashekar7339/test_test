...
def _plugin_module():
    ...
    return importlib.import_module(
        "fid_coder.plugins.prompt_newline.register_callbacks"
    )


def _config_module():
    return importlib.import_module("fid_coder.plugins.prompt_newline.config")
...
def test_patched_prompt_appends_newline_only_when_enabled():
    ...
    try:
        ...
        with patch(
            "fid_coder.plugins.prompt_newline.register_callbacks.is_enabled",
            return_value=False,
        ):
...
def test_handle_command_persists_explicit_on(tmp_path, monkeypatch):
    # Point the config at a throwaway file so we don't trash real fid.cfg
    ...
    with (
        patch(
            "fid_coder.plugins.prompt_newline.register_callbacks._emit_success"
        ) as mock_success,
        patch("fid_coder.plugins.prompt_newline.register_callbacks._emit_info"),
    ):
...
def test_handle_command_flips_when_no_arg(tmp_path, monkeypatch):
    ...
    with (
        patch("fid_coder.plugins.prompt_newline.register_callbacks._emit_success"),
        patch("fid_coder.plugins.prompt_newline.register_callbacks._emit_info"),
    ):
...
def test_handle_command_rejects_garbage_arg(tmp_path, monkeypatch):
    ...
    with patch(
        "fid_coder.plugins.prompt_newline.register_callbacks._emit_error"
    ) as mock_error:
