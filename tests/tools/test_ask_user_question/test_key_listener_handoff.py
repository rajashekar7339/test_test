...
from fid_coder.tools.ask_user_question.terminal_ui import interactive_question_picker
...
async def test_suspends_and_resumes_key_listener_when_present():
    ...
    with (
        patch(
            "fid_coder.agents._key_listeners.get_active_handle",
            return_value=fake_handle,
        ),
        patch(
            "fid_coder.tools.ask_user_question.tui_loop.run_question_tui",
            fake_run,
        ),
    ):
...
async def test_resume_still_called_when_suspend_timed_out():
    ...
    with (
        ...
        patch(
            "fid_coder.agents._key_listeners.emit_warning",
        ) as mock_warn,
        patch(
            "fid_coder.tools.ask_user_question.tui_loop.run_question_tui",
            fake_run,
