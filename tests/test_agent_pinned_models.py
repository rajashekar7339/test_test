...
def mock_config_paths(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_config_dir = os.path.join(tmp_dir, ".fid_coder")
        tmp_config_file = os.path.join(tmp_config_dir, "fid.cfg")
        monkeypatch.setattr("fid_coder.config.CONFIG_DIR", tmp_config_dir)
        monkeypatch.setattr("fid_coder.config.CONFIG_FILE", tmp_config_file)
...
class TestAgentPinnedModels:
    ...
    def test_base_agent_get_model_name(self):
        ...
        agent = FidCoderAgent()
        agent_name = agent.name  # "fid-coder"
