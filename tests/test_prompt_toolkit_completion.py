...
def test_set_completer_excludes_fid_token(monkeypatch):
    # Ensure 'fid_token' is a config key but SetCompleter doesn't offer it
    monkeypatch.setattr(
        "fid_coder.command_line.prompt_toolkit_completion.get_config_keys",
        lambda: ["fid_token", "user_name", "temp_dir"],
    )
    monkeypatch.setattr(
        "fid_coder.command_line.prompt_toolkit_completion.get_value",
        lambda key: "sensitive_token_value" if key == "fid_token" else "normal_value",
    )
    completer = SetCompleter()

    # Test with full "fid_token" typed
    doc = Document(text="/set fid_token", cursor_position=len("/set fid_token"))
    completions = list(completer.get_completions(doc, None))
    assert completions == [], (
        "SetCompleter should not complete for 'fid_token' key directly"
    )

    # Test with partial "fid" that would match "fid_token"
    doc = Document(text="/set fid", cursor_position=len("/set fid"))
    completions = list(completer.get_completions(doc, None))
    assert completions == [], (
        "SetCompleter should not complete for 'fid_token' key even partially"
    )
...
def test_set_completer_no_match(monkeypatch):
    monkeypatch.setattr("fid_coder.config.get_config_keys", lambda: ["actual_key"])
    completer = SetCompleter()
...
@pytest.mark.asyncio
@patch("fid_coder.command_line.prompt_toolkit_completion._NoGhostLinesPromptSession")
@patch("fid_coder.command_line.prompt_toolkit_completion.FileHistory")
@patch("fid_coder.command_line.prompt_toolkit_completion.merge_completers")
...
async def test_prompt_session_style_uses_central_theme_adapter(
    ...
    with (
        patch(
            "fid_coder.command_line.prompt_toolkit_completion.on_prompt_text_color",
            return_value=None,
        ),
        patch(
            "fid_coder.command_line.prompt_toolkit_completion.on_prompt_toolkit_style",
...
def test_meaningful_prompt_fragments_include_semantic_roles():
    agent = MagicMock(display_name="Fid Coder")
    ...
    assert "class:fid class:tui.header" in styles
...
@pytest.mark.asyncio
@patch("fid_coder.command_line.prompt_toolkit_completion._NoGhostLinesPromptSession")
@patch("fid_coder.command_line.prompt_toolkit_completion.SafeFileHistory")
