"""Tests for fid_coder.messaging.bottom_bar - the scroll-region manager."""
...
from fid_coder.messaging.bottom_bar import (
    PANEL_MAX_ROWS,
    RESERVED_ROWS,
    BottomBar,
    get_bottom_bar,
...
def test_prompt_buffer_uses_theme_foreground(bar, tty):
    with patch("fid_coder.callbacks.on_prompt_text_color", return_value="#6a9955"):
        bar.start()
        drain(tty)
...
def test_styled_prefix_restores_theme_foreground_for_user_text(bar, tty):
    ...
    with patch("fid_coder.callbacks.on_prompt_text_color", return_value="#4c4f69"):
        bar.start()
        drain(tty)
        bar.set_prompt_text(
...
def test_swarm_cancel_panel_clear_keeps_cursor_hidden(bar, tty, monkeypatch):
    ...
    drain(tty)
    monkeypatch.setattr("fid_coder.messaging.bottom_bar.get_bottom_bar", lambda: bar)
