...
class TestStderrCapture:
    ...
    def test_default_handler_logging(self):
        ...
        with patch("fid_coder.mcp_.captured_stdio_server.logger") as mock_logger:
            capture = StderrCapture("test-server")
...
class TestCapturedMCPServerStdio:
    ...
    async def test_client_streams_context_manager(self):
        ...
        with (
            ...
            patch(
                "fid_coder.mcp_.captured_stdio_server.stdio_client"
            ) as mock_stdio_client,
            patch(
                "fid_coder.mcp_.captured_stdio_server.StderrCapture"
    ...
    async def test_stderr_line_handler(self):
        ...
        from fid_coder.mcp_.captured_stdio_server import logger
...
class TestBlockingStartup:
    ...
    async def test_blocking_startup_exception_group_unwrapping(self):
        ...
        with (
            patch(
                "fid_coder.mcp_.blocking_startup.SimpleCapturedMCPServerStdio.__aenter__",
                side_effect=exc_group,
            ),
            patch("fid_coder.mcp_.blocking_startup.emit_info") as mock_emit,
        ):
