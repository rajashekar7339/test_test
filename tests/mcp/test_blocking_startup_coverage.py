...
class TestStderrFileCaptureMonitoring:
    ...
    @patch("fid_coder.mcp_.blocking_startup.emit_info")
...
class TestSimpleCapturedMCPServerStdioClientStreams:
    ...
    async def test_client_streams_creates_stderr_capture(self):
        ...
        with (
            patch("fid_coder.mcp_.blocking_startup.stdio_client", mock_stdio_client),
            patch("fid_coder.mcp_.blocking_startup.rotate_log_if_needed"),
            patch("fid_coder.mcp_.blocking_startup.get_log_file_path") as mock_get_path,
            patch("fid_coder.mcp_.blocking_startup.write_log"),
        ):
...
class TestStartupMonitorEdgeCases:
    ...
    async def test_wait_all_ready_reports_correct_counts(self):
        ...
        with patch(
            "fid_coder.mcp_.blocking_startup.emit_info", side_effect=capture_emit
        ):
