...
class TestAutostartBoundServers:
    def test_starts_stopped_server_when_auto_start_true(self, mock_manager):
        ...
        with patch(
            "fid_coder.mcp_.agent_bindings.get_bound_servers",
            return_value={"srv-1": {"auto_start": True}},
        ):
            _builder._autostart_bound_servers(mock_manager, "fid-coder")
    ...
    def test_no_bindings_is_noop(self, mock_manager):
        with patch("fid_coder.mcp_.agent_bindings.get_bound_servers", return_value={}):
            _builder._autostart_bound_servers(mock_manager, "fid-coder")
        mock_manager.start_server_sync.assert_not_called()
...
class TestMissingServerWarning:
    ...
    def test_warns_when_declared_server_not_installed(self, mock_manager):
        ...
        with (
            ...
            patch("fid_coder.agents._builder.emit_warning") as mock_warn,
        ):
...
class TestAutostartBoundServersAsync:
    ...
    async def test_awaits_start_for_stopped_server(self, mock_manager):
        ...
        with patch(
            ...
            await _builder.autostart_bound_servers_async(mock_manager, "fid-coder")
...
class TestLoadMcpServersWiring:
    ...
    def test_autostart_called_when_agent_name_given(self, mock_manager):
        with (
            patch(
                "fid_coder.agents._builder.get_mcp_manager", return_value=mock_manager
            ),
            patch(
                "fid_coder.agents._builder._autostart_bound_servers"
            ) as mock_autostart,
            patch("fid_coder.agents._builder.get_value", return_value=None),
        ):
            _builder.load_mcp_servers(agent_name="fid-coder")
        mock_autostart.assert_called_once_with(mock_manager, "fid-coder")
    ...
    def test_returns_empty_when_disabled(self, mock_manager):
        with (
            ...
            patch("fid_coder.agents._builder.get_value", return_value="true"),
        ):
            result = _builder.load_mcp_servers(agent_name="fid-coder")
        assert result == []
        mock_autostart.assert_not_called()
...
class TestSubagentInvocationUsesAsyncAutostart:
    ...
    def _code_without_comments() -> str:
        ...
        source = Path("fid_coder/tools/subagent_invocation.py").read_text(
            encoding="utf-8"
        )
...
class TestPreMcpAutostartHook:
    ...
    def test_sync_fires_hook_with_agent_and_server_names(self, mock_manager):
        ...
        with (
            ...
            patch("fid_coder.agents._builder.on_pre_mcp_autostart_sync") as mock_hook,
        ):
            _builder._autostart_bound_servers(mock_manager, "fid-coder")
        mock_hook.assert_called_once_with("fid-coder", ["srv-1"])
    ...
    async def test_async_fires_hook_with_agent_and_server_names(self, mock_manager):
        ...
        with (
            ...
            patch(
                "fid_coder.agents._builder.on_pre_mcp_autostart",
            ) as mock_hook,
        ):
