...
class TestHandleToolsCommand:
    ...
    def test_tools_command_with_markdown_rendering(self):
        ...
        with patch(
            "fid_coder.command_line.core_commands.tools_content", mock_tools_content
        ):
            with patch("fid_coder.messaging.emit_info") as mock_emit:
    ...
    def test_tools_command_with_empty_content(self):
        ...
        with patch("fid_coder.command_line.core_commands.tools_content", ""):
            with patch("fid_coder.messaging.emit_info") as mock_emit:
                result = handle_tools_command("/tools")
                assert result is True
                mock_emit.assert_called_once()
    ...
    def test_tools_command_with_unicode_content(self):
        ...
        with patch(
            "fid_coder.command_line.core_commands.tools_content", unicode_content
        ):
            with patch("fid_coder.messaging.emit_info") as mock_emit:
                result = handle_tools_command("/tools")
                assert result is True
...
class TestHandleExitCommand:
    ...
    def test_exit_command_with_emit_error(self):
        ...
        with patch(
            "fid_coder.messaging.emit_success", side_effect=Exception("Emit failed")
        ):
...
class TestHandleAgentCommand:
    ...
    def test_agent_command_show_current_with_descriptions(self):
        ...
        with patch(
            "fid_coder.command_line.core_commands.interactive_agent_picker",
            side_effect=Exception("Picker failed"),
        ):
            with patch("fid_coder.agents.get_current_agent", return_value=mock_current):
                with patch(
                    "fid_coder.agents.get_available_agents", return_value=mock_agents
                ):
                    with patch(
    ...
    def test_agent_command_successful_switch(self):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch("fid_coder.agents.set_current_agent", return_value=True):
                    with patch(
                        "fid_coder.config.finalize_autosave_session",
    ...
    def test_agent_command_with_agent_argument(self):
        ...
        with patch("fid_coder.agents.get_available_agents", return_value=mock_agents):
            with patch(
                "fid_coder.agents.get_current_agent",
    ...
    def test_agent_command_switch_failure_handling(self):
        ...
        with patch(
            "fid_coder.agents.get_available_agents", return_value={"target": "Target"}
        ):
            with patch("fid_coder.agents.get_current_agent", return_value=mock_current):
                with patch(
                    "fid_coder.agents.set_current_agent", return_value=False
                ):  # Switch fails
                    with patch(
                        "fid_coder.config.finalize_autosave_session",
...
class TestInteractiveAgentPicker:
    ...
    def test_agent_picker_returns_selected_agent(self):
        ...
        with patch(
            ...
            with patch("fid_coder.agents.get_current_agent") as mock_get_current:
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            "fid_coder.agents.set_current_agent", return_value=True
                        ):
                            with patch(
                                "fid_coder.config.finalize_autosave_session",
...
class TestHandleModelCommand:
    ...
    def test_model_command_interactive_success(self):
        ...
        with patch(
            "fid_coder.command_line.core_commands.interactive_model_picker",
            return_value="gpt-4",
        ):
            with patch(
    ...
    async def test_model_picker_displays_panel(
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "fid_coder.tools.common.arrow_select_async", return_value="gpt-3.5"
                ):
...
class TestHandleMcpCommand:
    ...
    def test_mcp_command_delegates_to_handler(self):
        ...
        with patch(
            "fid_coder.command_line.mcp.MCPCommandHandler"
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_agent_command_with_unicode_agent_name(self):
        ...
        with patch("fid_coder.messaging.emit_error"):
            with patch("fid_coder.agents.get_available_agents", return_value={}):
    ...
    def test_tools_command_with_malformed_markdown(self):
        ...
        with patch(
            "fid_coder.command_line.core_commands.tools_content", malformed_content
        ):
            with patch("fid_coder.messaging.emit_info") as mock_info:
                result = handle_tools_command("/tools")
                assert result is True
...
class TestIntegrationScenarios:
    ...
    def test_error_recovery_scenarios(self):
        ...
        with patch(
            ...
            with patch("fid_coder.agents.get_current_agent"):
                with patch(
