...
from fid_coder.command_line.mcp_completion import MCPCompleter, load_server_names


class TestLoadServerNames:
    @patch("fid_coder.mcp_.manager.MCPManager")
    ...
    def test_failure(self):
        with patch("fid_coder.mcp_.manager.MCPManager", side_effect=Exception("err")):
            result = load_server_names()
            assert result == []
...
class TestMCPCompleter:
    ...
    def test_get_server_names_refresh(self):
        ...
        with patch(
            "fid_coder.command_line.mcp_completion.load_server_names",
            return_value=["new"],
        ):
            result = self.completer._get_server_names()
            assert result == ["new"]
