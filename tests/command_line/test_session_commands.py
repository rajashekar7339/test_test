...
class TestGetCommandsHelp:
    def test_lazy_import(self):
        from fid_coder.command_line.session_commands import get_commands_help
...
class TestHandleSessionCommand:
    def _run(self, command):
        from fid_coder.command_line.session_commands import handle_session_command
    ...
    def test_session_new(self):
        with (
            ...
            patch("fid_coder.messaging.emit_success") as mock_s,
        ):
    ...
    def test_session_invalid(self):
        with patch("fid_coder.messaging.emit_warning") as mock_w:
...
class TestHandleCompactCommand:
    def _run(self, cmd="/compact"):
        from fid_coder.command_line.session_commands import handle_compact_command
    ...
    def test_mid_run_queues_compaction_for_next_model_call(self):
        ...
        with (
            patch("fid_coder.messaging.run_ui.is_run_active", return_value=True),
            patch(
                "fid_coder.messaging.pause_controller.get_pause_controller",
                return_value=controller,
            ),
            patch("fid_coder.messaging.emit_info") as emit_info,
        ):
    ...
    def test_no_history(self):
        ...
        with (
            ...
            patch("fid_coder.messaging.emit_warning") as mw,
        ):
            assert self._run() is True
    ...
    def test_truncation_strategy(self):
        ...
        with (
            ...
            patch("fid_coder.config.get_protected_token_count", return_value=50),
            patch("fid_coder.agents._compaction.truncate", return_value=["m3"]),
            patch("fid_coder.messaging.emit_info"),
            patch("fid_coder.messaging.emit_success") as ms,
        ):
            assert self._run() is True
    ...
    def test_compaction_fails(self):
        ...
        with (
            ...
            patch("fid_coder.config.get_protected_token_count", return_value=50),
            patch("fid_coder.messaging.emit_info"),
            patch("fid_coder.messaging.emit_error") as me,
        ):
            assert self._run() is True
    ...
    def test_zero_before_tokens(self):
        ...
        with (
            ...
            patch("fid_coder.config.get_protected_token_count", return_value=0),
            patch("fid_coder.messaging.emit_info"),
...
class TestHandleTruncateCommand:
    def _run(self, cmd):
        from fid_coder.command_line.session_commands import handle_truncate_command
...
class TestHandleAutosaveLoadCommand:
    def test_returns_marker(self):
        from fid_coder.command_line.session_commands import (
...
class TestHandleDumpContextCommand:
    ...
    def test_success(self):
        ...
        with (
            ...
            patch(
                "fid_coder.session_lifecycle.persist_named_session",
                return_value=meta,
            ),
            patch("fid_coder.messaging.emit_success"),
        ):
...
class TestHandleLoadContextCommand:
    ...
    def test_file_not_found_with_available(self):
        with (
            patch(
                "fid_coder.command_line.session_commands.load_session",
                side_effect=FileNotFoundError(),
            ),
            patch(
                "fid_coder.command_line.session_commands.list_sessions",
                return_value=["s1"],
            ),
            patch("fid_coder.messaging.emit_error"),
            patch("fid_coder.messaging.emit_info") as mi,
        ):
    ...
    def test_success(self):
        ...
        This is the deliberate, original ``/load_context`` semantic:
        ...
        with (
            ...
            patch("fid_coder.command_line.autosave_menu.display_resumed_history"),
        ):
...
class TestHandleClearCommand:
    ...
    def _run(self, command="/clear"):
        from fid_coder.command_line.session_commands import handle_clear_command
    ...
    def test_clear_wipes_history_and_rotates_session(self):
        ...
        with (
            ...
            patch(
                "fid_coder.config.finalize_autosave_session",
    ...
    def test_clear_is_registered_and_appears_in_help(self):
        ...
        import fid_coder.command_line.session_commands  # noqa: F401
